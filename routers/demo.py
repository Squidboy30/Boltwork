...
LNBITS_URL = os.environ.get("DEMO_LNBITS_URL", "").rstrip("/")
...
async def _get_balance() -> int:
    """Get demo wallet balance in sats."""
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        if r.status_code == 200:
            data = r.json()
            return data.get("balance", 0) // 1000  # msat to sat
    return 0


async def _pay_invoice(invoice: str) -> str:
    """Pay a bolt11 invoice from the demo wallet. Returns preimage."""
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if not payment_hash:
            raise HTTPException(status_code=500, detail="No payment hash returned")
