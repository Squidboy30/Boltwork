...
| Workflow Pipeline | `POST /workflow/run` | 1000 sats |
...
| Workflow Info | `GET /workflow/info` | Pipeline syntax and supported services |
...
Chain multiple services in a single call. Pay once, describe a pipeline, get the final result plus all intermediate outputs. Up to 5 steps. Use `{"$from": N}` to pass the primary output of step N into the next step.

```bash
# Fetch a webpage and translate the summary to French
curl -X POST https://parsebit.fly.dev/workflow/run \
  -H "Content-Type: application/json" \
  -d '{
    "label": "fetch and translate",
    "steps": [
...
Supported services: `webpage`, `pdf`, `summarise`, `translate`, `data`, `tables`, `explain`, `review`, `compare`.

Primary outputs passed by `$from`: `webpage/pdf/data/tables/compare/review` → `summary` · `translate` → `translated_text` · `explain` → `explanation`.
...
```json
{
  ...
  "step_results": [
    {"title": "...", "summary": "...", "key_points": [], "sentiment": "neutral", "topics": []},
    {"source_language": "English", "target_language": "French", "translated_text": "..."}
  ],
  "_meta": {"total_input_tokens": 1240, "total_output_tokens": 310, "model": "claude-sonnet-4-6", "duration_ms": 4200, "price_sats": 1000}
}
```
